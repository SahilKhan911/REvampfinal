...
class InfiniteGridMenu {
  ...
  #onControlUpdate(deltaTime) {
    ...
    // Cancel animated snap if user grabs the sphere
    if (this.control.isPointerDown && this._isAnimatingSnap) {
      this._isAnimatingSnap = false;
      this._snapTargetOrientation = null;
    }

    // Animated snap interpolation
    if (this._isAnimatingSnap && this._snapTargetOrientation) {
      const SNAP_SPEED = 0.08; // lower = slower/smoother
      quat.slerp(
        this.control.orientation,
        this.control.orientation,
        this._snapTargetOrientation,
        SNAP_SPEED
      );
      ...
      const dot = Math.abs(
        this.control.orientation[0] * this._snapTargetOrientation[0] +
        this.control.orientation[1] * this._snapTargetOrientation[1] +
        this.control.orientation[2] * this._snapTargetOrientation[2] +
        this.control.orientation[3] * this._snapTargetOrientation[3]
      );
      if (dot > 0.9999) {
        quat.copy(this.control.orientation, this._snapTargetOrientation);
        this._isAnimatingSnap = false;
        this._snapTargetOrientation = null;
      }
    }

    const isMoving = this.control.isPointerDown || this._isAnimatingSnap || Math.abs(this.smoothRotationVelocity) > 0.01;
  ...
  snapToItem(itemIndex) {
    const numItems = Math.max(1, this.items.length);
    // Find the first vertex that maps to this item
    let targetVertexIndex = -1;
    for (let i = 0; i < this.instancePositions.length; i++) {
      if (i % numItems === itemIndex) {
        targetVertexIndex = i;
        break;
      }
    }
    if (targetVertexIndex === -1) return;

    const localDir = vec3.normalize(vec3.create(), this.instancePositions[targetVertexIndex]);
    const targetDir = vec3.fromValues(0, 0, -1);

    const targetOrientation = quat.create();
    const cross = vec3.cross(vec3.create(), localDir, targetDir);
    const crossLen = vec3.length(cross);

    if (crossLen > 0.0001) {
      vec3.normalize(cross, cross);
      const dot = vec3.dot(localDir, targetDir);
      const angle = Math.acos(Math.max(-1, Math.min(1, dot)));
      quat.setAxisAngle(targetOrientation, cross, angle);
    } else if (vec3.dot(localDir, targetDir) < 0) {
      const perp = Math.abs(localDir[0]) < 0.9
        ? vec3.fromValues(1, 0, 0)
        : vec3.fromValues(0, 1, 0);
      const axis = vec3.cross(vec3.create(), localDir, perp);
      vec3.normalize(axis, axis);
      quat.setAxisAngle(targetOrientation, axis, Math.PI);
    }

    // Set target and begin animated slerp
    this._snapTargetOrientation = targetOrientation;
    this._isAnimatingSnap = true;

    // Kill residual velocity so the slerp is clean
...
const InfiniteMenu = forwardRef(function InfiniteMenu({ items = [], scale = 1.0, onActiveItemChange }, ref) {
  ...
  // Expose snapToItem to parent via ref
  useImperativeHandle(ref, () => ({
    snapToItem: (index) => {
      if (sketchRef.current) {
        sketchRef.current.snapToItem(index);
      }
    }
  }), []);
  ...
  useEffect(() => {
    ...
    const handleActiveItem = index => {
      ...
      if (onActiveItemChangeRef.current) onActiveItemChangeRef.current(itemIndex);
    };
